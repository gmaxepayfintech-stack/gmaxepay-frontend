...
const MasterDistribution = ({
  ...
  useEffect(() => {
    if (companyAepsStatus?.status === "SUCCESS") {
      ...
      if (embedded) return;
      const payload = {
        query: { userRole: 3 }, // Master Distributor role
        options: {
          sort: { id: -1 },
          page: currentPage,
          paginate: 6,
  ...
  const allTableData = useMemo(() => {
    ...
    if (Array.isArray(responseForTable) && responseForTable.length > 0) return responseForTable;
  ...
  const handleExportToExcel = () => {
    ...
    const fileName = `MasterDistributor_Export_${new Date().toISOString().split("T")[0]}.xlsx`;
    ...
    <div className={`text-[#1B1717] ${embedded ? "[&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]" : "min-h-screen p-4 sm:p-6"}`}>
      <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-4 mb-6">
          <h2 className="text-xl sm:text-2xl font-normal text-gray-800">Master Distributor</h2>
          <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center justify-end gap-3">
            ...
            <button onClick={handleExportToExcel} className="flex items-center justify-center gap-2 bg-[#039155] text-white px-4 py-3 rounded-lg font-[Gilroy-Medium] hover:bg-green-700 shadow-md text-sm sm:text-base">
...
<th className="px-3 py-4 font-[Gilroy-Medium] text-sm tracking-wider whitespace-nowrap">Status</th>
            ...
            <tbody className="bg-white divide-y text-center">
              {isLoading ? (
                <TableBodyLoader colSpan={20} />
              ) : !tableData || tableData.length === 0 ? (
                <tr><td colSpan={20} className="py-12 text-gray-500 font-[Gilroy-Medium]">No data available</td></tr>
              ) : (
                tableData.map((row, index) => (
                  <tr key={index} className={`text-sm ${index % 2 === 0 ? "bg-green-50" : "bg-white"}`}>
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <button onClick={() => { const userId = row.id || row.originalItem?.id; if (userId) { dispatch(getCompanyAdmin(userId)); setShowProfileDetails(true); } }} className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center hover:bg-gray-300">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <button onClick={() => { const userId = row.id || row.originalItem?.id; if (userId) dispatch(checkCompanyAepsStatus(userId)); }} disabled={row.aepsOnboardingStatus === true} className={`px-3 py-1 border border-indigo-500 text-indigo-600 rounded-lg text-xs ${row.aepsOnboardingStatus === true ? "opacity-50 cursor-not-allowed bg-gray-100" : "hover:bg-indigo-50"}`}>Check Status</button>
                    </td>
