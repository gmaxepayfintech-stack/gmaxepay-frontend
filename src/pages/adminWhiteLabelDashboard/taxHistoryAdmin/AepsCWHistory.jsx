...
const AepsCWHistory = ({ onBack = null, apiType = "aeps1", transactionType = "CW" }) => {
  ...
  const transformApiData = (dataArray) => {
    ...
    return dataArray.map((item, index) => {
      ...
      const formattedComm = `₹${Number(wlComm).toFixed(2)}`;
      const formattedTDS = `₹${Number(wlTDS).toFixed(2)}`;
  ...
  useEffect(() => {
    ...
    // Pass the status filter inside query
    if (statusFilter !== "All") {
  ...
  }, [dispatch, debouncedSearchQuery, statusFilter, fromDate, toDate, apiType, transactionType, currentPage, itemsPerPageState]);
  ...
  const handleExportToExcel = async () => {
    ...
    try {
      ...
      if (apiType === "aeps2") {
        result = await getAeps2CwHistoryCompany(payload)(customDispatch);
      } else {
        result = await getAepsCwHistoryCompany(payload)(customDispatch);
      }
            ...
            {!isLoading && (
              <tbody className="bg-white divide-y divide-gray-200">
                {paginatedTransactions.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={apiType === "aeps2" ? 22 : 21} className="px-4 sm:px-6 py-8 text-center">
