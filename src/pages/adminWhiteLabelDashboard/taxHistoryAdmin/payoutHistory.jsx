...
import { getPayoutHistoryCompany, payoutStatusCheckUser, payoutStatusCheckCompany } from "../../../redux/action/payoutAction";
...
const PayoutHistory = ({ onBack, type }) => {
  ...
  const handleCheckStatus = async (transaction) => {
    ...
    try {
      ...
      if (response?.status === "SUCCESS") {
        ...
        dispatch(getPayoutHistoryCompany({
          query,
          customSearch: {},
          options: {
            page: 1,
  ...
  const payoutHistoryResponse = useSelector(
    (state) => state?.payout?.payoutHistoryCompany,
  );
  ...
  useEffect(() => {
    ...
    dispatch(getPayoutHistoryCompany(payload));
