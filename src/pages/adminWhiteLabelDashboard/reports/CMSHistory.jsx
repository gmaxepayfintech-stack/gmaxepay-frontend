...
import { cmsCompanyHistory } from "../../../redux/action/rechargeAction";
...
const CMSHistory = ({ onBack }) => {
    ...
    const cmsCompanyReportsResponse = useSelector((state) => state?.recharge?.cmsCompanyReports);
    const apiData = cmsCompanyReportsResponse?.cmsCompanyReports || [];
    const paginator = cmsCompanyReportsResponse?.paginator || {};
    ...
    const fetchData = useCallback((page = 1) => {
        ...
        dispatch(cmsCompanyHistory(payload));
    ...
    const transformApiData = (dataArray) => {
        ...
        return dataArray.map((item, index) => {
            ...
            return {
                id: item.id || index, formattedDateTime,
    ...
    try {
      ...
      const result = await cmsCompanyHistory(payload)(customDispatch);
      let rawDocs = [];
      if (result) {
            ...
            <div className="mb-3 sm:mb-4">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 mb-2 sm:mb-3">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                        ...
                        <button onClick={() => { setFromDate(""); setToDate(""); setSearchQuery(""); setStatusFilter("All"); setCurrentPage(1); setIsReloading(true); dispatch(cmsCompanyHistory({ query: {}, customSearch: {}, options: { page: 1, paginate: itemsPerPage, sort: { createdAt: -1 } } })); }}
