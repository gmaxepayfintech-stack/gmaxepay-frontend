...
const KycModal = ({
    ...
    // Handle cases where kycData is the full response {status, data: {...}} or just the data object {...}
    ...
    // Extract all possible IDs from the response to handle mixed formats (numeric vs string)
    ...
    // Strict exact matching to prevent partial matches (e.g., ID "3" matching "35")
    ...
    return (
        <>
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[110] animate-fadeIn p-4 font-[Gilroy]">
                <div
                    ref={kycModalRef}
                    className="bg-white rounded-[24px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] w-full max-w-5xl max-h-[92vh] overflow-hidden animate-slideUp flex flex-col border border-slate-100 relative"
                >
                    {/* Professional Header */}
                    <div className="relative z-10 flex items-center justify-between p-6 border-b border-gray-100 bg-white">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-3 mt-0.5">
                                    <p className="text-xs font-[Gilroy-Medium] text-slate-500 uppercase tracking-wider">Retailer ID: {user.userId || user.userCode || data.userId || selectedUserId || 'N/A'}</p>
                                    <div className="w-1 h-1 bg-slate-300 rounded-full" />
                                    <div className="flex items-center gap-1">
                                        <Layers className="w-3 h-3 text-slate-400" />
                                        <span className="text-[10px] font-[Gilroy-Bold] text-slate-500">{data.completedSteps}/{data.totalSteps} Steps</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                            <div className="hidden sm:flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 mr-12">
                                ...
                                <span className="text-sm font-[Gilroy-Bold] text-slate-700">{user.name}</span>
                            </div>
                            <button
                                onClick={handleClose}
                                className="absolute right-4 top-4 w-8 h-8 flex items-center justify-center rounded-xl bg-[#039155] hover:opacity-90 transition"
                            >
                    ...
                    {/* Tab Navigation */}
                    <div className="relative z-10 px-6 bg-white border-b border-gray-100">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50 scrollbar-hide relative z-10">
                        {(() => {
                            ...
                            // Show loader if we are explicitly loading, OR if we have a selected ID but no matching data yet (and no failure)
                            const showLoader = loading || (selectedUserId && !effectiveKycData && !hasFailed);

                            if (showLoader) {
                                return (
                                    <div className="flex flex-col items-center justify-center py-20 gap-4">
                                        <ButtonLoader size={40} color="#039155" />
                            ...
                            if (effectiveKycData) {
                                return (
                                    ...
                                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4 group">
                                        <div className="flex items-center gap-4">
                                            <div className="w-16 h-16 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl flex items-center justify-center shadow-lg shadow-emerald-200/50 group-hover:scale-105 transition-transform duration-500">
                                                <UserIcon className="w-8 h-8 text-white" />
                                            </div>
                                            <div>
                                                <div className="text-2xl font-[Gilroy-Bold] text-slate-900 group-hover:text-[#039155] transition-colors">{user.name || 'N/A'}</div>
                                                <div className="flex items-center gap-2 text-slate-500 text-sm font-[Gilroy-Medium]">
                                                    <span className="bg-slate-100 px-2 py-0.5 rounded text-[10px] font-[Gilroy-Bold] text-slate-600 uppercase tracking-wider">{user.userRole === 3 ? 'Master Distributor' : user.userRole === 4 ? 'Distributor' : 'Retailer'}</span>
                                                    <span>•</span>
                                                    <span>{user.userId || user.userCode || user._id || user.id || selectedUserId || 'N/A'}</span>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div className="space-y-3">
                                                {[
                                                    ...
                                                    <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-slate-50/50 border border-slate-100">
                                                        <span className="text-slate-700 text-sm font-[Gilroy-Bold]">{step.label}</span>
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex items-center gap-2 mt-1">
                                                        <span className="text-[10px] text-slate-400 font-[Gilroy-Bold] uppercase tracking-wider">UID: {aadhaar.uid || 'Not Masked'}</span>
                                                        <div className="w-1 h-1 bg-slate-200 rounded-full" />
                                                        <span className="text-[10px] text-slate-400 font-[Gilroy-Bold] uppercase tracking-wider">DOB: {aadhaar.dob || 'N/A'}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <button
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            {[
                                                ...
                                                <div key={i} className="space-y-3">
                                                    ...
                                                    {doc.image ? (
                                                        <div className="relative group overflow-hidden rounded-2xl border border-slate-200 aspect-[1.6/1] bg-slate-50">
                                                            ...
                                                            <div className="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/40 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100">
                                                                <div className="bg-white p-3 rounded-full shadow-2xl scale-75 group-hover:scale-100 transition-transform">
                                                                    <ZoomIn className="text-slate-900 w-5 h-5" />
                                                                </div>
                                                            </div>
                                                        </div>
                                                    ) : (
                                                        <div className="aspect-[1.6/1] rounded-2xl bg-slate-100 flex items-center justify-center border-2 border-dashed border-slate-200 text-slate-400 text-xs font-[Gilroy-Bold]">No Image Found</div>
                                                    )}
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        <div className="space-y-6">
                                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-5 bg-slate-50 rounded-2xl border border-slate-100">
                                                {[
                                                    ...
                                                    <div key={i}>
                                                        <p className="text-[10px] text-slate-400 font-[Gilroy-Bold] uppercase mb-1 tracking-wider">{info.label}</p>
                                                ...
                                                {[
                                                    ...
                                                    <div key={i} className="flex items-start gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                                        ...
                                                        <div className="overflow-hidden">
                                                            <p className="text-[10px] text-slate-400 font-[Gilroy-Bold] uppercase tracking-wider mb-0.5">{info.label}</p>
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                    {[
                                                        ...
                                                        <div key={i} className="relative group overflow-hidden rounded-2xl border border-slate-200 aspect-[1.4/1] bg-slate-50 shadow-inner">
                                                            {doc.image ? (
                                                                <>
                                        ...
                                        {bank.accountNumber ? (
                                            <div className="p-8 bg-slate-50 rounded-2xl border border-slate-100 relative overflow-hidden group">
                                                <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:opacity-[0.05] transition-opacity">
                                                    <FaUniversity className="w-32 h-32 rotate-12" />
                                                </div>
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                                                    {[
                                                        ...
                                                        <div key={idx} className="space-y-1">
                                                            <p className="text-[10px] text-slate-400 font-[Gilroy-Bold] uppercase tracking-wider">{info.label}</p>
                                                            {info.isStatus ? (
                                                                <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-[Gilroy-Bold] bg-emerald-500 text-white uppercase tracking-widest shadow-sm">
                                                                    {info.value}
                                                                </span>
                                                            ) : (
            ...
            {showRevertConfirm && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] flex items-center justify-center z-[120] animate-fadeIn p-4 overflow-y-auto font-[Gilroy]">
                    <div
                        ...
                        className="bg-white rounded-2xl shadow-2xl w-full max-w-sm relative overflow-hidden animate-slideUp border border-slate-200"
                    >
                        <div className="p-6">
                            ...
                            <div className="flex gap-3">
                                <button
                                    ...
                                    className="flex-1 py-2.5 text-xs font-[Gilroy-Bold] text-slate-500 bg-slate-50 hover:bg-slate-100 rounded-lg transition-all"
                                >
                                    Cancel
                                </button>
                                <button
                                    ...
                                    disabled={isReverting}
                                    className="flex-1 py-2.5 bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition-all font-[Gilroy-Bold] text-xs flex items-center justify-center"
                                >
                                    {isReverting ? <ButtonLoader size={16} color="#ffffff" /> : "Confirm Revert"}
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            {confirmModal?.show && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] flex items-center justify-center z-[130] animate-fadeIn p-4 overflow-y-auto font-[Gilroy]">
                    <div
                        onClick={(e) => e.stopPropagation()}
                        className="bg-white rounded-2xl shadow-2xl w-full max-w-[400px] overflow-hidden animate-slideUp border border-slate-200"
                    >
                        <div className="p-6">
            ...
            {zoomedImage && (
                <div
                    className="fixed inset-0 bg-slate-900/98 backdrop-blur-xl flex items-center justify-center z-[130] animate-fadeIn p-4 cursor-zoom-out"
                    onClick={() => setZoomedImage(null)}
                >
                    <div className="relative max-w-full max-h-full flex items-center justify-center">
                        <button
                            onClick={() => setZoomedImage(null)}
                            className="absolute -top-14 right-0 text-white hover:text-gray-300 transition-colors p-3 bg-white/10 rounded-full"
                        >
                            <X className="w-6 h-6" />
                        </button>
                        <img
                            src={zoomedImage}
                            alt="Zoomed Document"
                            className="max-w-[95vw] max-h-[90vh] object-contain rounded-xl shadow-[0_0_100px_rgba(0,0,0,0.5)] border border-white/5 cursor-default"
                            onClick={(e) => e.stopPropagation()}
                        />
                    </div>
                </div>
