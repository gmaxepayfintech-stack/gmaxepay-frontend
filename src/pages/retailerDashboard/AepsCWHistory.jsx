...
const AepsCWHistory = ({ onBack = null, apiType = "aeps1", transactionType = "CW" }) => {
  ...
  const transformApiData = (dataArray) => {
    ...
    return dataArray.map((item, index) => {
      ...
      const formattedComm = `₹${Number(retComm).toFixed(2)}`;
      const formattedTDS = `₹${Number(retTDS).toFixed(2)}`;
  ...
  }, [dispatch, debouncedSearchQuery, fromDate, toDate, apiType, transactionType, statusFilter, currentPage, itemsPerPage]);
            ...
            {!isLoading && (
              <tbody className="bg-white divide-y divide-gray-200">
                {paginatedTransactions.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={apiType === "aeps2" ? 20 : 19} className="px-4 sm:px-6 py-8 text-center">
