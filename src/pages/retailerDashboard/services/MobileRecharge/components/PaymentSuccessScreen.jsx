...
const PaymentSuccessScreen = ({ transactionDetails, mobileNumber, selectedPlanForRecharge }) => {
  ...
  const generatePDF = () => {
    ...
    doc.text("Your Payment Has Been Completed", pageWidth / 2, yPos, { align: "center" });
    yPos += 15;
    ...
    doc.text(`₹ ${transactionDetails.amount}`, pageWidth / 2, yPos, { align: "center" });
    yPos += 20;
    ...
    const details = [
      { label: "Transaction ID", value: transactionDetails.transactionId || "N/A" },
      { label: "Mobile Number", value: transactionDetails.number || mobileNumber },
      { label: "Transaction Status", value: transactionDetails.status || "Success" },
      { label: "Validity", value: selectedPlanForRecharge?.validity || "N/A" },
      { label: "B-Connect Transaction ID", value: transactionDetails.bConnectId || "N/A" },
  ...
  const handleShare = async () => {
    try {
      ...
      const file = new File([pdfBlob], `Recharge_Receipt_${transactionDetails.orderid || Date.now()}.pdf`, {
