...
import { aepsThreeCashWithdrawl, aepsThreeBalanceEnquiry, aepsThreeMiniStatement, aepsThreeBankList, aepsThreeRecentBankList } from "../../../redux/action/aepsThreeAction";
...
const SelectserviceThree = () => {
    const dispatch = useDispatch();
    const { showNotification } = useNotification();
    const bankList = useSelector((state) => state.aepsThree?.bankList);
    const resentBankListState = useSelector(
        (state) => state.aepsThree?.bankRecentList,
    );
    ...
    const [modal, setModal] = useState({
        ...
        transactionType: null, // "cashWithdrawal" | "balanceEnquiry" | "miniStatement"
    });
    ...
    // Banks returned from recent banks API (aepsThreeRecentBankList)
    ...
    useEffect(() => {
        dispatch(aepsThreeRecentBankList({}))
            .then((response) => {
    ...
    const handleWithdrawal = async (values) => {
        ...
        try {
            ...
            if (errCode === "0") {
                ...
                try {
                    const response = await dispatch(aepsThreeCashWithdrawl(payload));
    ...
    const handleEnquiry = async (values) => {
        ...
        try {
            ...
            if (errCode === "0") {
                ...
                try {
                    const response = await dispatch(aepsThreeBalanceEnquiry(payload));
    ...
    const handleStatement = async (values) => {
        ...
        try {
            ...
            if (errCode === "0") {
                ...
                try {
                    const response = await dispatch(aepsThreeMiniStatement(payload));
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#D9D9D9CC] overflow-y-auto">
                    <div className="bg-green-100 rounded-xl relative overflow-hidden max-w-md w-full mx-4 shadow-xl my-auto" ref={receiptRef}>
                        ...
                        <div className="relative z-10 pt-12 pb-12 px-12">
                            ...
                            {transactionData && (
                                <div className={transactionType === 'miniStatement' ? 'mb-20 w-[480px]' : 'mb-20'}>
                                            ...
                                            <>
                                                ...
                                                {displayAmount != null && displayAmount !== '' && (
                                                    <div className="border-2 border-dashed border-[#1B1717] rounded-lg p-3 text-center mb-5">
                                                        <div className="text-[11px] font-['Gilroy-Medium'] text-[#1B1717]/60 mb-1">Withdrawal Amount</div>
                                                        <div className="text-[24px] font-['Gilroy-SemiBold'] text-[#1B1717]">₹{displayAmount}</div>
                                                    </div>
                                                )}
                                                {/* Details Grid */}
                                                {fields.length > 0 && (
                                            ...
                                            <>
                                                ...
                                                {balanceAmt != null && (
                                                    <div className="border-2 border-dashed border-[#039155] rounded-lg p-3 text-center mb-5">
                                                        <div className="text-[11px] font-['Gilroy-Medium'] text-[#1B1717]/60 mb-1">Available Balance</div>
                                        ...
                                        return (
                                            <>
                                                ...
                                                {stmtList.length > 0 ? (
                                                    ...
                                                    <p className="text-xs text-gray-400 font-['Gilroy-Regular'] text-center mt-4">No statement entries found.</p>
                                                )}
                                            </>
                                        );
                                    })()}
    ...
    return (
        <div className="w-full">
            ...
            <Modal
                isOpen={modal.isOpen}
                onClose={() =>
                    setModal({ ...modal, isOpen: false, transactionData: null, transactionType: null })
                }
                title={modal.title}
...
export default SelectserviceThree;
