...
const CMSHistory = ({ onBack }) => {
    ...
    // Build and dispatch API payload
    ...
    // Reset isReloading
    useEffect(() => {
        if (!isLoading && isReloading) setIsReloading(false);
    ...
    const transformApiData = (dataArray) => {
        ...
        return dataArray.map((item, index) => {
            ...
            const status =
                ...
                rawStatus === "PENDING" ? "Pending" : rawStatus;
            ...
            return {
                ...
                billerId: item.billerId || "N/A",
                transactionId: item.referenceId || "N/A",
                refNo: item.utr || item.ackno || "N/A",
                amount: `₹${item.amount ?? 0}`,
                commission: item.commission ?? 0,
                openingWallet: item.openingWallet,
                closingWallet: item.closingWallet,
                debit: item.debit ?? 0,
    ...
    try {
      ...
      const result = await cmsUserHistory(payload)(customDispatch);
      let rawDocs = [];
      if (result) {
    ...
    const handleViewClick = (transaction) => {
        setSelectedTransactionData(transaction.originalItem);
        setShowTransactionDetails(true);
    };

    if (showTransactionDetails) {
        return (
            <CMSTransactionDetails
                transactionData={selectedTransactionData}
                onBack={() => { setShowTransactionDetails(false); setSelectedTransactionData(null); }}
            />
        );
    }
    ...
    return (
        <div className="min-h-screen bg-[#FAFAFA] p-2 sm:p-3 md:p-4 text-[#1B1717] flex flex-col max-w-[1600px] mx-auto">
            ...
            <div className="p-1 mb-2 flex-shrink-0">
                <div className="flex flex-col md:flex-col lg:flex-row items-stretch lg:items-end gap-3 sm:gap-4">
                    <div className="relative flex-1">
                        ...
                        <input type="text" placeholder="Search by TXN ID / Reference ID" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
            ...
            <div className="bg-white rounded-xl sm:rounded-3xl shadow-sm mt-6 overflow-hidden flex-1">
                <div className="w-full overflow-x-auto overscroll-x-contain">
                    <table className="w-full border-collapse min-w-full">
                        <thead className="bg-[#FFFFFF] border-b border-gray-200">
                            <tr>
                                {["SR.No", "Date & Time", "TXN User", "Mobile No", "Event", "Biller Name", "TXN ID", "UTR / Ackno", "Amount", "Status", "Action"].map((h) => (
                        ...
                        {!isLoading && (
                            <tbody className="bg-white divide-y divide-gray-200">
                                {transactions.length > 0 ? (
                                        ...
                                        <tr key={transaction.id} className={`transition-colors ${index % 2 === 0 ? "bg-[#039155]/5 hover:bg-[#E8F5ED]" : "bg-white hover:bg-gray-50"}`}>
                                            ...
                                            <td className="px-4 sm:px-5 py-3 sm:py-4 whitespace-nowrap">
                                                <div className="flex flex-col">
                                                    <span className="text-xs sm:text-sm font-['Gilroy-Regular'] text-[#121216]">{transaction.txnUser}</span>
                                                    <span className="text-xs text-[#1B1717]/50">{transaction.userId}</span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-4 sm:px-5 py-3 sm:py-4 whitespace-nowrap">
                                                <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-[Gilroy-Medium] ${transaction.event === "CMS_POSTING" ? "bg-blue-100 text-blue-700" : "bg-purple-100 text-purple-700"}`}>
                                                    {transaction.event}
                                                </span>
                                            </td>
                                            ...
                                            <td className="px-4 sm:px-5 py-3 sm:py-4 whitespace-nowrap">
                                                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs sm:text-sm font-[Gilroy-Medium] ${transaction.status === "Success" ? "bg-[#039155] text-white" : transaction.status === "Pending" ? "bg-orange-500/80 text-white" : "bg-red-500/80 text-white"}`}>
                                                    {transaction.status}
                                                </span>
                                            </td>
                                            <td className="px-4 sm:px-5 py-3 sm:py-4 whitespace-nowrap">
                                    ...
                                    <tr><td colSpan={11} className="px-4 sm:px-6 py-8 text-center">
                                        <p className="text-sm sm:text-base font-['Gilroy-Medium'] text-gray-500">No transactions found</p>
                                    </td></tr>
                                )}
                            </tbody>
                        )}
                ...
                {/* Loading / Pagination */}
                {isLoading ? (
                    <div className="flex items-center justify-center gap-3 pt-4 pb-4 border-t border-gray-200">
                        <ButtonLoader color="#039155" size={24} thickness={3} />
                        <p className="text-sm sm:text-base font-['Gilroy-Medium'] text-[#1B1717]">Loading...</p>
                    </div>
                ) : (
                    <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200">
                        <p className="text-xs sm:text-sm text-[#1B1717]/60 font-['Gilroy-Regular']">
                            Showing {totalCount === 0 ? 0 : startIndex + 1}–{Math.min(startIndex + itemsPerPage, totalCount)} of {totalCount} records
                        </p>
                            ...
                            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                                ...
                                return (
                                    <button key={pageNum} onClick={() => setCurrentPage(pageNum)}
                                        className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg font-[Gilroy-Medium] transition text-sm sm:text-base ${currentPage === pageNum ? "bg-[#039155] text-white" : "bg-white border border-gray-300 text-[#1B1717] hover:bg-gray-50"}`}>
                                        {pageNum}
                                    </button>
                                );
                            })}
