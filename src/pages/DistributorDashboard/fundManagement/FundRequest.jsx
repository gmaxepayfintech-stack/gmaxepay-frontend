...
const FundRequest = () => {
  ...
  const fetchFundRequests = useCallback(async () => {
    ...
    try {
      ...
      if (result?.status === "SUCCESS" && result?.dFundrequest) {
        const dataArray = Array.isArray(result.dFundrequest) 
          ? result.dFundrequest 
          : result.dFundrequest?.data || [];
        setFundRequests(dataArray);
        const calculatedPages = Math.ceil(dataArray.length / itemsPerPage) || 1;
        setTotalPages(result.dFundrequest?.totalPages || calculatedPages);
        setTotalRecords(result.dFundrequest?.totalRecords || dataArray.length);
      }
    } catch (error) {
