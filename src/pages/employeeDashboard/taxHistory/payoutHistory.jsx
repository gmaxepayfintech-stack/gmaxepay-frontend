...
import { employeePayoutHistory, payoutStatusCheckUser, payoutStatusCheckCompany } from "../../../redux/action/payoutAction";
...
const PayoutHistory = ({ onBack, type }) => {
  ...
  const handleCheckStatus = async (transaction) => {
    ...
    try {
      ...
      if (response?.status === "SUCCESS") {
        ...
        dispatch(employeePayoutHistory({
          query,
          customSearch: {},
          options: {
            page: 1,
  ...
  const handleExportToExcel = async () => {
    ...
    try {
      ...
      const result = await employeePayoutHistory(payload)(customDispatch);
    ...
    const excelData = filteredExport.map((row, index) => ({
      ...
      "Opening Balance": stripRupee(row.openingBalance),
      "Closing Balance": stripRupee(row.closingBalance),
      "Date": row.createdAt,
    }));
            ...
            {!isLoading && (
              <tbody className="bg-white divide-y divide-gray-200 text-center">
                {paginatedTransactions.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={19} className="px-4 sm:px-6 py-8 text-center">
