...
const BillerSettings = () => {
    ...
    const {
        billers: reduxBillers,
        loading: reduxLoading,
        billersTotalPages: reduxTotalPages,
        billersCurrentPage,
        categoriesForDropdown: reduxCategories,
        paymentInfo,
        createBillerSuccess,
        updateBillerSuccess,
    ...
    useEffect(() => {
        if (paymentInfo && paymentInfo.length > 0) {
            const uniqueChannels = [
                ...new Set(
                    paymentInfo.map((item) => item.initiatingChannel).filter(Boolean),
                ),
            ];
    ...
    const handleAddBiller = async (formData) => {
        ...
        const selectedCategory = categoriesForDropdown.find(
    ...
    const handleToggleActive = async (biller, newActiveValue) => {
        ...
        const categoryObj = categoriesForDropdown.find(
    ...
    useEffect(() => {
        if (
            (createBillerSuccess || updateBillerSuccess) &&
            !loading &&
            isModalOpen &&
            lastOperation
        ) {
            ...
            {(billersTotalPages || 0) > 0 && (
                <div className="flex justify-center items-center gap-2 mt-8">
                    ...
                    <button
                        onClick={() =>
                            setCurrentPage((prev) =>
                                Math.min(billersTotalPages || 1, prev + 1),
                            )
                        }
