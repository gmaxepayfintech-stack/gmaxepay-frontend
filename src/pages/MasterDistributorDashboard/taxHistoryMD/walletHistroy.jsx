...
const WalletHistory = ({ onBack, type }) => {
  ...
  const paginator = walletHistoryResponse?.data?.paginator || walletHistoryResponse?.paginator || {};
  const totalCount = walletHistoryResponse?.data?.total || walletHistoryResponse?.total || filteredTransactions.length;
  const totalPages = paginator.pageCount || walletHistoryResponse?.data?.pages || Math.ceil(totalCount / itemsPerPage) || 1;
