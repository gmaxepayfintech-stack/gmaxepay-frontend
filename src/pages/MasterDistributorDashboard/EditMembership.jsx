...
const EditMembership = ({ scheme = null, onBack }) => {
    ...
    if (commData && Array.isArray(commData)) {
      const mapped = commData.map((item, index) => {
        ...
        return {
          ...
          // MD (Master Distributor) - ENT Margin
  ...
  const handleSaveCommissionRow = async (commission) => {
    try {
      ...
      if (!isMdChanged && !isDiChanged && !isReChanged) {
        return;
      }
      ...
      if (isMdChanged && commission.mdRoleId) {
        requests.push(
          dispatch(
            updateUserSlabCommission(companyId, commission.mdRoleId, {
      ...
      if (isDiChanged && commission.diRoleId) {
        requests.push(
          dispatch(
            updateUserSlabCommission(companyId, commission.diRoleId, {
      ...
      if (isReChanged && commission.reRoleId) {
        requests.push(
          dispatch(
            updateUserSlabCommission(companyId, commission.reRoleId, {
                ...
                <div className="min-w-[1000px]">
                  {commLoading && items.length === 0 ? (
                    ...
                    items.map((commission, index) => {
                      ...
                      const isDirty = isMdChanged || isDiChanged || isReChanged;
                      ...
                      return (
                        <div key={commission.id || index}>
                          <div className="grid grid-cols-7 gap-4 px-4 py-3 hover:bg-gray-50 items-center">
                            ...
                            {/* ENT Margin (MD) - shows Master Distributor commission */}
                            {renderCommissionInput("masterDistributor", "ENT Margin")}
