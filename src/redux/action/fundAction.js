...
export const companyCreditDebit = (payload) => async (dispatch) => {
  dispatch({ type: LOADING_START });
  try {
    const authToken = secureLocalStorage.getItem("userToken");
    const token = typeof authToken === "string" ? authToken : String(authToken || "");
    ...
    const response = await axios.post(
      `${API_ROUTE}/api/v1/company/fund/credit-debit`,
      payload,
      {
        headers: {
          "Content-Type": "application/json",
    ...
    if (status === "SUCCESS") {
      dispatch({
        type: COMPANY_CREDIT_DEBIT_SUCCESS,
        payload: { status, message },
      });
      return { status, message };
    } else {
      dispatch({
        type: COMPANY_CREDIT_DEBIT_FAILURE,
        payload: {
          status: response?.data?.status ?? "FAILURE",
          message: response?.data?.message ?? commonError,
        },
